<template>
  ...
  <!--<el-button @click="submit">数据</el-button>-->
      ...
      <el-form :model="form">
        ...
        <el-form-item label="店铺" :label-width="formLabelWidth" >
          <el-select v-model="form.shop_id" placeholder="选择店铺" >
            <el-option v-for="data in shopData" :key="data.shop_id" :label="data.shop_name" :value="data.shop_id"></el-option> 
          </el-select>
        </el-form-item>
...
//1、设置 action http://10.9.151.159:8000/admin/uploadImg
...
export default {
  ...
  computed:{
    ...mapGetters(['shopData','deClassData'])
  },
  
  methods: {
    ...mapActions(["addPro","getShopList",'getdeClassData']),
    ...
    uploadSuc(response, file, fileList){
      ...
      //response 后端返回的数据
      var url = "http://127.0.0.1:3000/"+ response.imgSrc.substr(6) //拼接图片地址
    ...
    submitPro(){
      ...
      console.log(this.form) //需要提交给后端的所有商品数据
      this.addPro({...this.form,callback:()=>{
        this.$message({
          message: '添加商品成功',
          type: 'success'
        });
  ...
  mounted(){
      this.getShopList({pageSize:10,pageNum:1})
      this.getdeClassData({pageSize:100,pageNum:1})
    }
}
